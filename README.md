# Minecraft Bedrock Launcher

Быстрый и стильный Android лаунчер для Minecraft Bedrock Edition с автоматическим распознаванием установленных версий.

## 📥 Скачать APK

- **Скачать напрямую из репозитория:** [apk/MinecraftLauncher.apk](https://github.com/AndreyDev86/Qwe/raw/main/apk/MinecraftLauncher.apk)
- **Скачать из раздела Releases:** [app-debug.apk](https://github.com/AndreyDev86/Qwe/releases/download/latest/app-debug.apk)

[![Скачать APK](https://img.shields.io/badge/Скачать_APK-MinecraftLauncher.apk-38BDF8?style=for-the-badge&logo=android&logoColor=white)](https://github.com/AndreyDev86/Qwe/raw/main/apk/MinecraftLauncher.apk)

Или перейдите в раздел [Releases (Релизы)](https://github.com/AndreyDev86/Qwe/releases).

---

## ⚡ Особенности
- 🔍 **Автоматическое распознавание** версий Minecraft (Bedrock, Preview, Education Edition) и популярных клонов / песочниц (Craftsman, LokiCraft, MultiCraft, RealmCraft, PojavLauncher, Master Craft, Survivalcraft и др.).
- 🚀 **Быстрый запуск** выбранной версии в один клик.
- 🎨 **Современный неоновый интерфейс** в стиле Material 3 с отображением имени приложения, версии и типа.
- 📲 **Кнопка "Поделиться"** для быстрой отправки лаунчера друзьям.
